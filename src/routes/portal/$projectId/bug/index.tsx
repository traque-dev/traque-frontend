...
import { captureBugByProjectId } from '@/api/bugs';
...
export const Route = createFileRoute('/portal/$projectId/bug/')({
      ...
      try {
        ...
        if (attachedFiles.length > 0) {
          const uploaded = await Promise.all(
            attachedFiles.map((file) => uploadFile({ file, purpose: 'BUG' })),
          );
          fileIds = uploaded.map((f) => f.id);
        }

        await captureBugByProjectId(projectId, {
          title: value.title,
